...
/** Mejor tráiler de YouTube para una peli/serie (o null si no hay). */
export async function fetchTrailer(
  mediaType: MediaType,
  id: number,
): Promise<Trailer | null> {
  const res = await fetch(`/api/tmdb/videos?mediaType=${mediaType}&id=${id}`);
  if (!res.ok) return null;
  const data = (await res.json()) as { trailer: Trailer | null };
  return data.trailer;
}
